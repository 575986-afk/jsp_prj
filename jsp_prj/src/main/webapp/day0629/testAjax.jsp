...
function requestXML(){
	$.ajax({
		...
		success: function(xmlDoc){
			...
			$("#output").html("<strong>"+$(xmlDoc).find("msg").text()+"</stong>");
		}
	});//ajax
}
function requestJSON(){
	$.ajax({
		...
		success: function(jsonObj){
			...
			$("#output").html("<strong>"+jsonObj.name+"</stong><br>"+jsonObj.age+" / "+jsonObj.addr);
		}
	});//ajax
}
</script>
...
<body>
	...
	<main>
		<div style="margin-top: 30px">
			<h3>AJAX요청에 대한 응답</h3>
			<input type="button" class="btn btn-success btn-sm" value="HTML요청"
				id="btnHTML" /> <input type="button" class="btn btn-primary btn-sm"
				value="TEXT요청" id="btnTEXT" /> <input type="button"
				class="btn btn-warning btn-sm" value="XML요청" id="btnXML" /> <input
				type="button" class="btn btn-info btn-sm" value="JSON요청"
				id="btnJSON" />
		</div>
		<select id="subject">
