...
public class SiteProperty {
	...
	static{
		//JDBC: DBCP와 관련 없이 사이트의 공통 정보를 로딩=>DBCP가 문제가 발생하더라도
		//사이트의 공통부분을 사용자에게 제공할 수 있다.
		...
		try {
			con=gc.getConn("dbcp");
			String selectSiteProperty="select protocal,server_name,server_port,context_root,upload_dir,api_key,site_info from site_property where type=1";
			...
			if(rs.next()) {
				spVO=new SitePropertyVO(rs.getString("protocal"),
